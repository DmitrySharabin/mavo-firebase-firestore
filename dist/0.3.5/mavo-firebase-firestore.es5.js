...
(function ($) {
  ...
  const _ = Mavo.Backend.register($.Class({
    ...
    constructor: function constructor(url, {
      ...
      this.features = _.getOptions(template, this.features); // If either of the auth providers is specified, we must enable the auth feature
      ...
      $.load("https://www.gstatic.com/firebasejs/7.8.2/firebase-app.js").then(async () => {
        ...
        $.include(!this.features.storage, "https://www.gstatic.com/firebasejs/7.8.2/firebase-storage.js"), // Authentication
        $.include(!this.features.auth, "https://www.gstatic.com/firebasejs/7.8.2/firebase-auth.js")]); // Get the config info from the attribute value

        $.extend(this, _.parseSource(this.source, this.defaults)); // The app's Firebase configuration

        const config = {
          apiKey: mavo.element.getAttribute("mv-firebase-key"),
          databaseURL: "https://".concat(this.projectId, ".firebaseio.com"),
          projectId: this.projectId,
          authDomain: "".concat(this.projectId, ".firebaseapp.com"),
          storageBucket: "".concat(this.projectId, ".appspot.com")
        ...
        if (!firebase.apps.length) {
          ...
          this.app = firebase.apps.find(app => app.options.projectId === this.projectId) || firebase.initializeApp(config, this.projectId);
        ...
        if (this.features.realtime) {
          ...
          this.unsubscribe = this.db.doc(this.filename).onSnapshot(doc => _.updatesHandler(doc, mavo), error => mavo.error("Firebase Realtime: ".concat(error.message)));
    ...
    load: function load() {
      ...
      return this.ready.then(() => this.get(this.filename).then(doc => Promise.resolve(doc.data() || {})).catch(error => {
     ...
     */
    put: function put(serialized, path = this.path, o = {}) {
      // Since we support offline persistence, we don't want end-users to think an app is hung when we are offline.
    ...
    upload: function upload(file, path) {
      ...
      return this.put(file, path, {
        isFile: true
      }).then(downloadURL => downloadURL).catch(error => {
        if (error.code) {
          if (this.features.auth) {
            Mavo.warn(this.mavo._("firebase-check-security-rules"));
          } else {
    ...
    static: {
      ...
      parseSource: function parseSource(source, defaults = {}) {
        const ret = {};
       ...
       */
      getAuthProviders: function getAuthProviders(template, defaults) {
        var _template2;
       ...
       */
      buildProvider: function buildProvider(provider) {
        // Fallback to Google
        provider = provider || "google"; // Make provider name title-cased
