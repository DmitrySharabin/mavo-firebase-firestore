...
(function ($) {
  ...
  const _ = Mavo.Backend.register($.Class({
    ...
    upload: function upload(file, path) {
      ...
      return this.put(file, path).then(downloadURL => downloadURL).catch(error => {
        if (error.code) {
          if (this.features.auth) {
            Mavo.warn(this.mavo._("firebase-check-security-rules"));
          } else {
    ...
    store: async function store(data, {
      path,
      format = this.format
    } = {}) {
      // Since we support offline persistence, we don't want end-users to think an app is hung when we are offline.
      ...
      return this.db.doc(this.filename).set(JSON.parse(serialized)).then(() => {
