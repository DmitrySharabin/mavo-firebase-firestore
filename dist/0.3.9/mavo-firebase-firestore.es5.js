...
(function ($) {
  ...
  Mavo.Plugins.register("firebase-firestore", {
    dependencies: ["https://cdn.jsdelivr.net/gh/DmitrySharabin/mavo-firebase-firestore/mavo-firebase-firestore.css"],
    hooks: {
      "init-start": function initStart(mavo) {
        ...
        Object.keys(PROVIDERS).forEach(p => {
          const id = "firebase-auth-".concat(p);
          Mavo.UI.Bar.controls[id] = {
            ...
            condition() {
              var _mavo$primaryBackend$, _mavo$primaryBackend$2;

              return Boolean(mavo.primaryBackend.project) && ((_mavo$primaryBackend$ = mavo.primaryBackend.authProviders) === null || _mavo$primaryBackend$ === void 0 ? void 0 : (_mavo$primaryBackend$2 = _mavo$primaryBackend$.includes) === null || _mavo$primaryBackend$2 === void 0 ? void 0 : _mavo$primaryBackend$2.call(_mavo$primaryBackend$, p));
            }
        ...
        $.extend(Mavo.UI.Bar.controls.login, {
          condition() {
            var _mavo$primaryBackend$3;

            return Boolean(mavo.primaryBackend.project) ? !((_mavo$primaryBackend$3 = mavo.primaryBackend.authProviders) === null || _mavo$primaryBackend$3 === void 0 ? void 0 : _mavo$primaryBackend$3.length) : mavo.primaryBackend.permissions.login;
          }
  ...
  const _ = Mavo.Backend.register($.Class({
    ...
    constructor(url, o) {
      ...
      $.extend(this, this.defaults); // Warn an author about deprecated attributes
      ...
      for (const attribute in DEPRECATED) {
        if (o.mavo.element.hasAttribute(attribute)) {
          ...
          Mavo.warn("@".concat(attribute, " is deprecated. Please use @").concat(newAttribute, " instead. For details, see ").concat(url, "."));
        }
      } // Which backend features should we support?


      const template = o.options || o.mavo.element.getAttribute("mv-firebase") || "";
      this.features = _.getOptions(template, this.defaults.features); // If either of the auth providers is specified, we must enable the auth feature
      ...
      if (this.features.auth) {
        this.permissions.on("login"); // If none of the auth providers is specified, by default Google is used
      ...
      $.load("https://www.gstatic.com/firebasejs/8.2.0/firebase-app.js").then(async () => {
        var _o$project, _o$collection, _o$filename, _this$key;

        // Then download the other parts if needed
        await Promise.all([// Cloud Firestore
        $.load("https://www.gstatic.com/firebasejs/8.2.0/firebase-firestore.js"), // Cloud Storage
        $.include(!this.features.storage, "https://www.gstatic.com/firebasejs/8.2.0/firebase-storage.js"), // Authentication
        $.include(!this.features.auth, "https://www.gstatic.com/firebasejs/8.2.0/firebase-auth.js")]); // Get the config info from the attribute value

        $.extend(this, _.parseSource(this.source, this.defaults)); // If an author provided backend metadata, use them
        // since they have higher priority

        this.project = (_o$project = o.project) !== null && _o$project !== void 0 ? _o$project : this.project;
        this.collection = (_o$collection = o.collection) !== null && _o$collection !== void 0 ? _o$collection : this.collection;
        this.filename = (_o$filename = o.filename) !== null && _o$filename !== void 0 ? _o$filename : this.filename; // The app's Firebase configuration

        const config = {
          apiKey: (_this$key = this.key) !== null && _this$key !== void 0 ? _this$key : o.mavo.element.getAttribute("mv-firebase-key"),
          databaseURL: "https://".concat(this.project, ".firebaseio.com"),
          projectId: this.project,
          authDomain: "".concat(this.project, ".firebaseapp.com"),
          storageBucket: "".concat(this.project, ".appspot.com")
        ...
        if (!firebase.apps.length) {
          ...
          this.app = firebase.apps.find(app => app.options.projectId === this.project) || firebase.initializeApp(config, this.project);
        }
        ...
        if (this.features["offline-persistence"]) {
          ...
          try {
            ...
            if (error.code === "unimplemented") {
              ...
              this.mavo.error("Firebase Offline Persistence: ".concat(error.message));
            }
          }
        }
        ...
        if (this.features.realtime) {
          ...
          this.unsubscribe = this.db.doc(this.filename).onSnapshot(doc => _.updatesHandler(doc, o.mavo), error => o.mavo.error("Firebase Realtime: ".concat(error.message)));
        ...
        if (this.features.auth) {
          const defaultPermissions = ["login"]; // By default, if the authentication feature is on, only signed-in users can edit and save the app's data.
          ...
          this.app.auth().onAuthStateChanged(user => {
            if (user) {
              ...
              // User is signed out
              this.user = null;
              $.fire(o.mavo.element, "mv-logout", {
                backend: this
              });
              this.permissions.off(["edit", "add", "delete", "save", "logout"]).on(defaultPermissions);
            }
          });
        }
    ...
    update(url, o) {
      var _o$project2, _o$collection2, _o$filename2;

      this.super.update.call(this, url, o);
      $.extend(this, _.parseSource(this.source, this.defaults)); // If an author provided backend metadata, use them
      // since they have higher priority

      this.project = (_o$project2 = o.project) !== null && _o$project2 !== void 0 ? _o$project2 : this.project;
      this.collection = (_o$collection2 = o.collection) !== null && _o$collection2 !== void 0 ? _o$collection2 : this.collection;
      this.filename = (_o$filename2 = o.filename) !== null && _o$filename2 !== void 0 ? _o$filename2 : this.filename;
    ...
    async load() {
      ...
      try {
        ...
        this.mavo.error("Firebase Load Data: ".concat(error.message));
        return null;
      }
    },
    ...
    put(serialized, path = this.path, o = {}) {
      ...
      if (o.isFile) {
        ...
        return this.storageBucketRef.child(path).put(serialized).then(snapshot => snapshot.ref.getDownloadURL());
      }

      return this.db.doc(this.filename).set(JSON.parse(serialized)).then(() => Promise.resolve()).catch(error => {
        if (this.features.auth) {
          Mavo.warn(this.mavo._("firebase-check-security-rules"));
        } else {
        ...
        this.mavo.error("Firebase Auth: ".concat(error.message));
      });
    },
    ...
    async upload(file, path) {
      path = "".concat(this.bucketName, "/").concat(path);

      try {
        const url = await this.put(file, path, {
          isFile: true
        });
        return url;
      } catch (error) {
        ...
        this.mavo.error("".concat(error.message));
        return null;
      }
    },
    ...
    async login(passive) {
      ...
      return new Promise((resolve, reject) => {
        if (passive) {
          ...
          this.app.auth().signInWithPopup(_.buildProvider(this.provider)).catch(error => {
            this.mavo.error("Firebase Auth: ".concat(error.message));
            reject(error);
          });
        }
    ...
    logout() {
      return this.app.auth().signOut().catch(error => {
        this.mavo.error("Firebase Auth: ".concat(error.message));
      });
    },
    ...
    static: {
      ...
      parseSource(source, defaults = {}) {
        ...
        if (source.indexOf("/") > -1) {
          ...
          ret.project = ret.project || parts.shift(); // If source without project ID has an odd number of parts,
      ...
      getOptions(template, defaults) {
        ...
        if (template = (_template = template) === null || _template === void 0 ? void 0 : _template.trim()) {
          let ids = template.split(/\s+/); // Drop duplicates (last one wins)

          ids = Mavo.Functions.unique(ids.reverse()).reverse();
          all.forEach(id => ids.includes(id) ? ret[id] = true : ret[id] = false);
          return ret;
      ...
      updatesHandler(snapshot, mavo) {
        const source = snapshot.metadata.hasPendingWrites ? "Local" : "Server"; // TODO: There's the problem of what to do when local edits conflict with pulled data
      ...
      getAuthProviders(template, defaults) {
        ...
        if (template = (_template2 = template) === null || _template2 === void 0 ? void 0 : _template2.trim()) {
          let ids = template.split(/\s+/); // Convert all auth providers names to lowercase and drop duplicates

          ids = Mavo.Functions.unique(ids.map(id => id.toLowerCase())); // Drop not supported auth providers
      ...
      buildProvider(provider = "google") {
        ...
        return eval("new firebase.auth.".concat(provider, "AuthProvider()"));
      }
